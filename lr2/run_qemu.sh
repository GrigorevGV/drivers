...
# Скрипт для запуска образа BeagleBone Black в QEMU
# Использование: ./run_qemu.sh [путь_к_buildroot_output_images]

# Определяем путь к образам
if [ -n "$1" ]; then
    IMAGES_DIR="$1"
else
